...
The marketing/docs website for **Oz** ([oz.app](https://oz.app)), a terminal-first AI-native dev workspace. The product itself lives in a separate repo (`codecollab-co/oz`); this repo is only the landing page + docs site. It's statically exported and deployed to GitHub Pages under the `/oz-website` subpath — there is no backend, no server runtime, no database.
...
**Static export, subpath-hosted.** `next.config.mjs` sets `output: 'export'`, `trailingSlash: true`, and `basePath: '/oz-website'` (deployed at `codecollab-co.github.io/oz-website`, not a custom domain root). Any hardcoded internal link, asset path, or image `src` must be prefixed with the basePath (see the icon URLs in `app/layout.tsx` for the pattern: `/oz-website/favicon.ico`). Images are `unoptimized: true` since there's no image-optimization server in a static export. Deploy is `.github/workflows/deploy.yml`: on push to `main`, it runs `pnpm build` and publishes `out/` to GitHub Pages.
